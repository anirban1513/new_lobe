...
import os
from utils import file_name_path, resize_image_itkwithsize, ConvertitkTrunctedValue, getRangImageRange
...
def preparesampling3dtraindata(heart_imagepath, heart_labelpath, trainImage, trainMask, shape=(96, 96, 96)):
    ...
    for subsetindex in range(len(image_path_list)):
        ...
        filepath1 = os.path.join(trainImage + "/" + str(subsetindex) + ".npy")
        filepath = os.path.join(trainMask + "/" + str(subsetindex) + ".npy")
...
def preparetraindata():
    ...
    :return:
    """
    heart_path = "/data/lobedata/LUNG_LOBE_CT_Data/Train"
    image_name = "Image"
    mask_name = "Mask"
